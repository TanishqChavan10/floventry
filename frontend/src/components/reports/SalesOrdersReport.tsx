...
const STATUS_LABELS: Record<string, string> = { DRAFT: 'Draft', CONFIRMED: 'Confirmed', CLOSED: 'Closed', CANCELLED: 'Cancelled' };
...
export function SalesOrdersReport() {
  const { data, loading } = useQuery(GET_SALES_ORDERS, { fetchPolicy: 'cache-and-network' });

  const orders: Array<{
    id: string; order_number: string; status: string; customer_name: string; created_at: string;
    items: Array<{ id: string; ordered_quantity: number; issued_quantity: number; pending_quantity: number; product: { name: string; sku: string } }>;
  }> = data?.salesOrders ?? [];
  ...
  const customerData = useMemo(() => {
    ...
    return Object.values(m).sort((a, b) => b.orders - a.orders).slice(0, 10)
      .map((c) => ({ name: c.name.length > 16 ? c.name.slice(0, 16) + '...' : c.name, orders: c.orders }));
  }, [orders]);

  const fulfillment = useMemo(() => {
    let ordered = 0, issued = 0, pending = 0;
  ...
  const rate = fulfillment.ordered > 0 ? Math.round((fulfillment.issued / fulfillment.ordered) * 100) : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
            {loading && !data ? <Skeleton className="h-[200px] w-full" /> :
             pieData.length === 0 ? <div className="flex items-center justify-center h-[200px] text-sm text-muted-foreground">No orders found</div> :
            <>
        ...
        <Card>
          ...
          <CardContent>
            {loading && !data ? <Skeleton className="h-[200px] w-full" /> :
             customerData.length === 0 ? <div className="flex items-center justify-center h-[200px] text-sm text-muted-foreground">No data</div> :
        ...
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xl font-bold text-[var(--chart-2)]">{fulfillment.issued.toLocaleString()}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground">Pending</p>
              <p className="text-xl font-bold text-[var(--chart-4)]">{fulfillment.pending.toLocaleString()}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      {productDemand.length > 0 && (
        <Card>
          ...
          <CardContent>
            <ChartContainer config={demandConfig} className="w-full" style={{ height: Math.max(180, productDemand.length * 40) }}>
              <BarChart
                data={productDemand.map((p) => ({
                  name: p.name.length > 20 ? p.name.slice(0, 20) + '...' : p.name,
                  ordered: p.ordered, issued: p.issued,
                }))}
                layout="vertical"
              >
                <CartesianGrid horizontal={false} vertical={false} />
                <XAxis type="number" tickLine={false} axisLine={false} className="text-xs" />
                <YAxis dataKey="name" type="category" width={130} tickLine={false} axisLine={false} className="text-xs" />
                <ChartTooltip content={<ChartTooltipContent />} />
                <Legend />
                <Bar dataKey="ordered" fill="var(--color-ordered)" name="Ordered" />
                <Bar dataKey="issued" fill="var(--color-issued)" name="Issued" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ChartContainer>
          </CardContent>
        </Card>
