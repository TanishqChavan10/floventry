...
export function AdjustmentsReport() {
  ...
  const { data: byWarehouseData, loading: byWarehouseLoading } = useQuery(GET_ADJUSTMENTS_BY_WAREHOUSE, {
    variables: { days }, fetchPolicy: 'cache-and-network',
  });
  const { data: byUserData, loading: byUserLoading } = useQuery(GET_ADJUSTMENTS_BY_USER, {
    variables: { days, limit: 10 }, fetchPolicy: 'cache-and-network',
  });
  ...
  const warehouseData = (byWarehouseData?.adjustmentsByWarehouse ?? []).map((d: any) => ({
    name: String(d.warehouseName).length > 16 ? String(d.warehouseName).slice(0, 16) + '...' : d.warehouseName,
    adjustments: Number(d.totalAdjustments ?? 0),
  }));

  const userRows: Array<{ userId: string; userName: string; adjustmentCount: number; totalQuantity: number }> = byUserData?.adjustmentsByUser ?? [];
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-xl font-bold mt-1 text-[var(--chart-2)]">+{totalIn.toLocaleString()}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Units Removed</p>
            <p className="text-xl font-bold mt-1 text-[var(--chart-1)]">-{totalOut.toLocaleString()}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {trendsLoading && !trendsData ? <Skeleton className="h-[260px] w-full" /> :
           trendData.length === 0 ? <div className="flex items-center justify-center h-[260px] text-sm text-muted-foreground">No data for this period</div> :
          <ChartContainer config={trendConfig} className="h-[260px] w-full">
            <LineChart data={trendData}>
              ...
              <Legend />
              <Line type="monotone" dataKey="adjIn" stroke="var(--color-adjIn)" strokeWidth={2} dot={false} name="Adj. In" />
              <Line type="monotone" dataKey="adjOut" stroke="var(--color-adjOut)" strokeWidth={2} dot={false} name="Adj. Out" />
            </LineChart>
          </ChartContainer>}
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
            {byWarehouseLoading && !byWarehouseData ? <Skeleton className="h-[220px] w-full" /> :
             warehouseData.length === 0 ? <div className="flex items-center justify-center h-[220px] text-sm text-muted-foreground">No data</div> :
        ...
        <Card>
          ...
          <CardContent>
            {byUserLoading && !byUserData ? <Skeleton className="h-[220px] w-full" /> :
             userRows.length === 0 ? <div className="flex items-center justify-center h-[220px] text-sm text-muted-foreground">No data</div> :
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {userRows.map((row, i) => (
                  <TableRow key={row.userId}>
                    ...
                    <TableCell className="text-right font-medium">{row.adjustmentCount}</TableCell>
                    <TableCell className="text-right text-muted-foreground">{row.totalQuantity.toLocaleString()}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>}
