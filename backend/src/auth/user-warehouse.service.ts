...
export class UserWarehouseService {
  ...
  async assignUserToWarehouse(
    ...
    if (assignedByRole === Role.MANAGER) {
      const managerAccess = await this.canUserManageWarehouse(
        assignedBy,
        warehouseId,
      );
      if (!managerAccess) {
        throw new ForbiddenException(
          'You do not have permission to assign users to this warehouse',
        );
      }
    }
