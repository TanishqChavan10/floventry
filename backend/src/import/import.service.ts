...
import { LotSourceType } from '../inventory/stock-lot/entities/stock-lot.entity';
import * as Papa from 'papaparse';
import { BarcodeService } from '../inventory/barcode/barcode.service';
import { BarcodeFormatService } from '../inventory/barcode/barcode-format.service';
...
export class ImportService {
  ...
  async validateProductImport(
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const inputBarcodeRaw =
        typeof row.barcode === 'string' && row.barcode.trim().length > 0
          ? row.barcode
          : null;
      ...
      if (inputBarcodeRaw) {
        try {
          ...
          if (key && reservedBarcodes.has(key)) {
            ...
            // Persist canonical value so validation + execution behave consistently.
            row.barcode = canonical;
          }
        } catch (e: any) {
          errors.push({
            rowNumber,
            field: 'barcode',
            message: e?.message || 'Invalid barcode',
          });
        }
      }
        ...
        try {
          ...
          const product = queryRunner.manager.create(Product, {
            ...
            cost_price: row.data.cost_price
              ...
              : null,
            selling_price: row.data.selling_price
              ? parseFloat(row.data.selling_price)
              : null,
  ...
  async validateUnitImport(
    ...
    const existingShortCodes = new Set(
      existingUnits.map((u) => u.shortCode.toLowerCase()),
    );
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (row.isDefault && row.isDefault !== '') {
        ...
        if (
          !['true', 'false', 'yes', 'no', '1', '0'].includes(normalizedValue)
        ) {
          errors.push({
            rowNumber,
            field: 'isDefault',
