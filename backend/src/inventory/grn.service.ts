...
export class GRNService {
    ...
    async createGRN(
        ...
        if (po.status !== PurchaseOrderStatus.ORDERED) {
            throw new BadRequestException('Can only create GRN for ORDERED purchase orders');
        }

        // Validate warehouse matches PO
        if (po.warehouse_id !== input.warehouse_id) {
            throw new BadRequestException('Warehouse must match purchase order warehouse');
        }
        ...
        for (const item of input.items) {
            ...
            if (!poItem) {
                throw new BadRequestException(`Invalid purchase order item: ${item.purchase_order_item_id}`);
            }

            // Calculate remaining quantity
            const remainingQty = poItem.ordered_quantity - (poItem.received_quantity || 0);
        ...
        const items = input.items.map((itemInput) => {
            const poItem = po.items.find((i) => i.id === itemInput.purchase_order_item_id);
            if (!poItem) {
        ...
        try {
            ...
            if (po) {
                const fullyReceived = po.items.every(
                    (item) => Math.floor(item.received_quantity) >= Math.floor(item.ordered_quantity)
                );
